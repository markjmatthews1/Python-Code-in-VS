...
def start_dashboard(historical_data, filtered_df, tickers, dashboard_ranks):
    ...
    # Simple helper functions for data processing
    def get_last_n_with_datetime(df, ticker, n):
        """Simple helper to get last N rows for a ticker"""
        if df.empty:
            return df
        ticker_data = df[df["Ticker"] == ticker].copy()
        if ticker_data.empty:
            return ticker_data
        return ticker_data.sort_values("Datetime").tail(n).copy()
    
    def get_adx_rolling_data(df, tickers, min_points=100):
        """Simple helper for ADX data"""
        if df.empty:
            return df
        result = []
        for ticker in tickers:
    ...
    def update_dash(n, selected_tickers, n_clicks,
        ...
        try:
            ...
            # Build DataFrames for each chart type using simple helper functions
            ...
            adx_plot_df = get_adx_rolling_data(df, valid_ticker_rows, min_points=100)
            
            # For PMO, we need enough data for 35-period calculations
            pmo_plot_df = get_adx_rolling_data(df, valid_ticker_rows, min_points=50)
            ...
            # Create ADX and PMO data using the same logic as price/volume
            adx_plot_df = pd.concat([get_last_n_with_datetime(df, t, adx_tick_count) for t in valid_ticker_rows], ignore_index=True)
            pmo_plot_df = pd.concat([get_last_n_with_datetime(df, t, pmo_tick_count) for t in valid_ticker_rows], ignore_index=True)
            ...
            # Connect directly to working chart creation code
            print(f"🎨 CHART DEBUG: Creating price chart with {num_tickers} rows for tickers: {valid_ticker_rows}")
            print(f"🎨 CHART DEBUG: price_plot_df shape: {price_plot_df.shape if 'price_plot_df' in locals() else 'NOT DEFINED'}")
            ...
            # --- Working chart creation starts here ---
            print(f"🎨 CHART DEBUG: Creating price chart with {num_tickers} rows for tickers: {valid_ticker_rows}")
            print(f"🎨 CHART DEBUG: price_plot_df shape: {price_plot_df.shape if 'price_plot_df' in locals() else 'NOT DEFINED'}")
            ...
            price_fig = make_subplots(
                ...
                subplot_titles=[f"{ticker} Price" for ticker in valid_ticker_rows],
                row_heights=[1]*num_tickers
            )
            
            # Continue with chart building loop
            chart_success = False
            for i, ticker in enumerate(valid_ticker_rows, start=1):
                ticker_data = price_plot_df[price_plot_df["Ticker"] == ticker].copy()
                if ticker_data.empty:
                    print(f"⚠️ No data for {ticker} in price_plot_df")
                    continue
                
                print(f"✅ Processing {ticker} with {len(ticker_data)} data points")
                ...
                price_fig.add_trace(fig, row=i, col=1)
                chart_success = True
        ...
        # Connect to working volume chart creation
        ...
        for i, ticker in enumerate(valid_ticker_rows, start=1):
            ...
            if not vol_df.empty:
                volume_fig.add_trace(
                    go.Histogram(x=vol_df["TickVolume"], name=f"{ticker} Volume", nbinsx=50),
                    row=i, col=1
                )

        # Continue with ADX and PMO charts...
        adx_fig = go.Figure()
        pmo_fig = go.Figure()
        
        # Create news and whale tables
        news_table = html.Div("News data placeholder")
        whale_table = html.Div("Whale data placeholder")
    ...
    def update_trade_log(update_clicks, log_clicks, selected_rows, trade_type, ticker, qty, open_dt, open_price, close_dt, close_price, notes, stored_data):
        # Simplified trade log function
        return [], [], [], "", "", "", "", "", "", "", "", html.Div("Trade log placeholder")

# COMMENTED OUT ORPHANED CODE - TODO: CLEAN UP
