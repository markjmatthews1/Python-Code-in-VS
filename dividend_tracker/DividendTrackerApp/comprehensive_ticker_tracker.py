...
def load_ticker_reference_data(data_file):
    ...
    if not os.path.exists(data_file):
        ...
        sample_tickers = {
            'ABR': {'account': 'E*TRADE', 'quantity': 1050, 'price_paid': 13.88, 'payment_cycle': 'Quarterly'},
            'ACP': {'account': 'E*TRADE', 'quantity': 1625, 'price_paid': 6.64, 'payment_cycle': 'Monthly'},
            'AGNC': {'account': 'E*TRADE', 'quantity': 736, 'price_paid': 10.37, 'payment_cycle': 'Monthly'},
            'BITO': {'account': 'E*TRADE', 'quantity': 440, 'price_paid': 28.00, 'payment_cycle': 'Monthly'},
            'BRSP': {'account': 'E*TRADE', 'quantity': 568, 'price_paid': 5.70, 'payment_cycle': 'Quarterly'},
            'CHMI': {'account': 'E*TRADE', 'quantity': 2425, 'price_paid': 3.40, 'payment_cycle': 'Quarterly'},
            'DSL': {'account': 'E*TRADE', 'quantity': 260, 'price_paid': 12.50, 'payment_cycle': 'Monthly'},
            'DX': {'account': 'E*TRADE', 'quantity': 625, 'price_paid': 11.90, 'payment_cycle': 'Monthly'},
            'ECC': {'account': 'E*TRADE', 'quantity': 1142, 'price_paid': 10.10, 'payment_cycle': 'Monthly'},
            'EIC': {'account': 'E*TRADE', 'quantity': 466, 'price_paid': 15.65, 'payment_cycle': 'Monthly'},
            'MORT': {'account': 'E*TRADE', 'quantity': 90, 'price_paid': 11.30, 'payment_cycle': 'Quarterly'},
            'NHS': {'account': 'E*TRADE', 'quantity': 917, 'price_paid': 8.11, 'payment_cycle': 'Monthly'},
            'OFS': {'account': 'E*TRADE', 'quantity': 1385, 'price_paid': 9.53, 'payment_cycle': 'Quarterly'},
            'PDI': {'account': 'E*TRADE', 'quantity': 688, 'price_paid': 19.15, 'payment_cycle': 'Monthly'},
            'QDTE': {'account': 'E*TRADE', 'quantity': 276, 'price_paid': 38.83, 'payment_cycle': 'Weekly'},
            'QYLD': {'account': 'E*TRADE', 'quantity': 350, 'price_paid': 17.92, 'payment_cycle': 'Monthly'},
            'RYLD': {'account': 'E*TRADE', 'quantity': 579, 'price_paid': 16.54, 'payment_cycle': 'Monthly'},
            'SVOL': {'account': 'E*TRADE', 'quantity': 552, 'price_paid': 22.33, 'payment_cycle': 'Monthly'},
            # Schwab positions
            'DX_Schwab': {'account': 'Schwab', 'quantity': 125, 'price_paid': 11.91, 'payment_cycle': 'Monthly'},
            'AGNC_Schwab': {'account': 'Schwab', 'quantity': 1060, 'price_paid': 9.42, 'payment_cycle': 'Monthly'},
            'QDTE_Schwab': {'account': 'Schwab', 'quantity': 285, 'price_paid': 35.30, 'payment_cycle': 'Weekly'},
            'ECC_Schwab': {'account': 'Schwab', 'quantity': 1330, 'price_paid': 7.43, 'payment_cycle': 'Monthly'}
        }
...
def create_enhanced_ticker_sheet(ticker_structure, live_data, output_file):
    ...
    headers = [
        ...
        'Total Gain %', 'Payment Cycle', 'Rate Per Share', 'Original Payment', 
        'New Payment', 'Original Yield %'
    ]
    ...
    for ticker, structure_data in ticker_structure.items():
        # Get live data for this ticker
        clean_ticker = ticker.replace('_Schwab', '').replace('_E*TRADE', '')
        live_ticker_data = live_data.get(clean_ticker, {})
        ...
        # Row data matching actual Excel headers
        row_data = [
            clean_ticker,                           # Ticker
            ...
            dividend_per_share,                    # Rate Per Share
            original_value * (previous_yield / 100) if current_yield > 0 else 0,  # Original Payment
            current_value * (current_yield / 100) if current_yield > 0 else 0,    # New Payment
            previous_yield,                        # Original Yield %
        ]
        ...
        for col, value in enumerate(row_data, 1):
            ...
            if col in [4, 5, 6, 7, 9, 10, 11, 15, 16]:  # Currency columns (Price Paid $, Current Price $, Day Change $, etc.)
                if isinstance(value, (int, float)) and value != 0:
                    cell.number_format = '"$"#,##0.00'
                elif isinstance(value, (int, float)) and value == 0:
                    cell.value = ""  # Clean up cosmetic zeros
            elif col in [8, 12, 17]:  # Percentage columns (Change %, Total Gain %, Original Yield %)
                if isinstance(value, (int, float)) and value != 0:
                    cell.number_format = '0.00%'
                elif isinstance(value, (int, float)) and value == 0:
                    cell.value = ""  # Clean up cosmetic zeros
        ...
        # Note: monthly_dividend and annual_dividend removed from totals since they don't exist in new structure
    ...
    print(f"   📊 Total Original Value: ${totals['original_value']:,.2f}")
    print(f"   📊 Total Gain: ${totals['total_gain']:,.2f}")
