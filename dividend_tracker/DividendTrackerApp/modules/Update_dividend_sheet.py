        ...
        for index, row in enumerate(sheet.iter_rows(min_row=2, values_only=False), start=2):
            ...
            try:
                ...
                if 'QuoteResponse' in quote_json and 'QuoteData' in quote_json['QuoteResponse'] and isinstance(quote_json['QuoteResponse']['QuoteData'], list):
                    ...
                    if current_yield:
                        sheet.cell(row=index, column=15).value = round(float(current_yield), 2)  # CORRECTED: Column 15
                    else:
                        sheet.cell(row=index, column=15).value = current_yield

                    # CORRECTED: Compare column 15 (new) vs column 14 (beginning)
                    col_q_value = sheet.cell(row=index, column=15).value  # New yield
                    col_p_value = sheet.cell(row=index, column=14).value  # Beginning yield
                    try:
                        ...
                        sheet.cell(row=index, column=15).fill = fill  # CORRECTED: Apply color to column 15
...
# Integrated ticker tracking function
def update_integrated_ticker_tracking():
    """
    Automatically update the ticker tracking system after dividend sheet update
    """
    print("\n🔗 Auto-updating integrated ticker tracking system...")
    
    try:
        import sys
        import os
        
        # Add parent directory to path to access our ticker tracking modules
        parent_dir = os.path.dirname(os.path.dirname(__file__))
        if parent_dir not in sys.path:
            sys.path.append(parent_dir)
        
        # Import our ticker tracking integration
        from complete_ticker_integration import update_ticker_with_yield_history
        
        # Run the ticker tracking update
        print("   📊 Running ticker analysis with yield history...")
        update_ticker_with_yield_history()
        
        print("   ✅ Ticker tracking integration completed!")
        
    except ImportError as e:
        print(f"   ⚠️ Ticker tracking not available: {e}")
        print("   📝 Note: Run manually with complete_ticker_integration.py")
    except Exception as e:
        print(f"   ❌ Error in ticker tracking integration: {e}")

# At the very end of your script
if __name__ == "__main__":
    update_dividend_sheet()
    
    # Automatically update ticker tracking system
    update_integrated_ticker_tracking()
    
    print("✅ Complete update finished. Exiting.")
