...
def main():
    """
    Main function to update estimated income using weekly column format
    
    This preserves existing weekly data by adding new data to the next available column
    instead of overwriting existing data with ticker breakdown format.
    """
    try:
        print("📊 Running estimated income tracker...")
        print("💡 Using weekly column format (preserves historical data)")
        ...
        try:
            # Import the fixed weekly update function
            sys.path.insert(0, r"C:\Users\mjmat\Python Code in VS")
            from fixed_estimated_income_update import update_estimated_income_sheet
            
            # Run the weekly column update (preserves existing data)
            print("🔄 Running weekly estimated income update...")
            success = update_estimated_income_sheet()
            
            if success:
                print("✅ Weekly estimated income update completed successfully!")
                print("📅 New weekly data added to next available column")
                return os.path.join(parent_dir, "outputs", "Dividends_2025.xlsx")
            else:
                print("❌ Weekly estimated income update failed")
                # Fall back to basic update
                return fallback_estimated_income_update()
                
        except ImportError as e:
            print(f"⚠️ Import error with fixed updater: {e}")
...
def create_estimated_income_sheet(wb, use_api=False, use_hybrid=False):
    ...
    # Check if sheet exists and has data - DON'T DELETE if it has manually updated data
    existing_sheet = None
                ...
                if week_date in historical_portfolio and account in historical_portfolio[week_date]:
                    ...
                    if account in ['Schwab Individual', 'Schwab IRA', '401k Retirement'] and value == 0:
                        if account == 'Schwab IRA':
                            value = 0.00  # No hardcoded values - requires API integration
                        elif account == 'Schwab Individual':
                            value = 0.00  # No hardcoded values - requires API integration
                        elif account == '401k Retirement':
                            value = 0.00  # Will be provided via user input, not hardcoded
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        for arg in sys.argv[1:]:
            ...
            elif arg == "--no-enhanced":
                use_enhanced_sheets = False
    ...
    build_estimated_income_tracker(import_historical=import_historical, use_api=use_api, use_hybrid=use_hybrid, include_portfolio=include_portfolio, create_comprehensive=create_comprehensive, use_enhanced_sheets=use_enhanced_sheets)