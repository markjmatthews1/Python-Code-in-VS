#!/usr/bin/env python3
"""
Enhanced E*TRADE Accounts Historic Yield Updater for DividendTracker App
========================================================================

This module is a DividendTracker-integrated version of Update_dividend_sheet.py
Updates the "Accounts dividend historical yield" sheet in Dividends_2025.xlsx with current yield data
for all account sections:
- Etrade IRA
- Etrade Taxable  
- Schwab IRA
- Schwab Individual
...
class ETradeHistoricYieldUpdater:
    ...
    def __init__(self):
        ...
        self.sheet_name = "Accounts Div historical yield"
        self.session = None
        self.base_url = None
        
        # Define account sections based on enhanced structure (adjusted for shifted layout)
        self.account_sections = [
            ...
            {
                "name": "Etrade Taxable", 
                "ticker_start_row": 25,  # Adjusted for shifted layout
                "ticker_end_row": 36,    # Adjusted for shifted layout
                "average_row": 37        # Adjusted for shifted layout
            },
            {
                "name": "Schwab IRA",
                "ticker_start_row": 41,  # Corrected: AGNC starts at row 41
                "ticker_end_row": 44,    # Corrected: ECC ends at row 44
                "average_row": 45        # Adjusted for shifted layout
            },
            {
                "name": "Schwab Individual",
                "ticker_start_row": 49,  # Corrected: DX starts at row 49
                "ticker_end_row": 50,    # Corrected: QDTE ends at row 50
                "average_row": 51        # Adjusted for shifted layout
            }
        ]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if ('QuoteResponse' in quote_json and 
                    ...
                    # Get yield from API
                    api_yield = all_data.get('yield', 0)
                    
                    # If API yield is 0 but we have dividend data, use dividend as yield percentage
                    if api_yield == 0:
                        dividend = all_data.get('dividend', 0) or all_data.get('declaredDividend', 0)
                        
                        if dividend > 0:
                            # For some ETFs like QDTE, the 'dividend' field contains the yield as a decimal
                            # Convert to percentage: 0.2044 -> 20.44%
                            calculated_yield = dividend * 100
                            api_yield = calculated_yield
                            print(f"   📊 Using dividend field as yield for {symbol}: {calculated_yield:.4f}%")
    ...
    def update_historic_yield_sheet(self):
        ...
        try:
            ...
            # Open the workbook and get the target sheet
            ...
            # Insert new column after 'Beginning Dividend Yield' (column 15/O)
            insert_col = 16  # This will be column P after insertion
            today = date.today().strftime("%m-%d-%Y")
            
            print(f"📅 Inserting new yield column after 'Beginning Dividend Yield' at column {insert_col}")
            sheet.insert_cols(insert_col)
            
            # Copy formatting from adjacent column to maintain visual consistency
            print("🎨 Applying consistent formatting to new column...")
            orange_fill = PatternFill(start_color="FFC000", end_color="FFC000", fill_type="solid")
            
            # Apply formatting to title rows and separators (adjusted for shifted layout)
            title_and_separator_rows = [1, 22, 23, 38, 39, 46, 47, 52]  # Updated for current layout
            for row_num in title_and_separator_rows:
                if row_num <= sheet.max_row:
                    cell = sheet.cell(row=row_num, column=insert_col)
                    if not cell.value:  # Only apply to empty cells (title/separator rows)
            ...
            for account in self.account_sections:
                ...
                # Process ticker rows in this account
                for row_idx in range(account["ticker_start_row"], account["ticker_end_row"] + 1):
                    ticker_cell = sheet.cell(row=row_idx, column=1)  # Column A contains ticker symbol
                    ticker = ticker_cell.value
                    
                    if ticker and isinstance(ticker, str) and len(ticker.strip()) > 0:
                        ticker = ticker.strip().upper()
                        
                        # Skip header rows (they contain words like "TICKER")
                        if ticker in ['TICKER', 'QTY', 'PRICE', 'QTY #', 'PRICE PAID $']:
                            continue
                ...
                try:
                    ...
                    print(f"   ✅ Added average calculation for {account['name']}")
...
if __name__ == "__main__":
    print("*** TESTING: Enhanced E*TRADE Historic Yield Updater")
    success = run_historic_yield_update()
    
    if success:
        print("\n🎉 Historic yield update completed successfully!")
    else:
        print("\n[ERROR] Historic yield update failed!")
