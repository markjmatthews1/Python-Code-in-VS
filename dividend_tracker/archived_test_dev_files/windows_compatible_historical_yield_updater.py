#!/usr/bin/env python3
"""
Windows-Compatible Enhanced Historical Yield Updater
====================================================

Fixes Unicode/emoji issues for Windows subprocess execution.
All functionality remains the same, just removes emoji characters.
"""
...
class WindowsCompatibleHistoricalYieldUpdater:
    """Windows-compatible updater without emoji characters"""
    ...
    def load_cache_data(self):
        ...
        try:
            if not os.path.exists(self.cache_file):
                print("ERROR: Cache file not found:", self.cache_file)
                return None, None
                
            with open(self.cache_file, 'r') as f:
                cache_data = json.load(f)
                
            print("SUCCESS: Cache loaded:", cache_data.get('timestamp', 'No timestamp'))
            
            # Extract ticker yields and positions
            ticker_yields = cache_data.get('ticker_yields', {})
            positions = cache_data.get('positions', {})
            
            print(f"INFO: Found yields for {len(ticker_yields)} tickers")
            print(f"INFO: Found positions for {len(positions)} account groups")
    ...
    def find_account_groups_and_boundaries(self, ws):
        """Find account groups and their end boundaries - updated for current structure"""
        account_info = {}
        ...
        for row in range(1, min(80, ws.max_row + 1)):
            ...
            # Look for account group headers - updated patterns
        ...
        # Find end boundaries for each group based on current structure
        ...
        for i, group_name in enumerate(group_order):
            ...
            if i < len(group_order) - 1:
                ...
                while end_row <= ws.max_row:
                    ...
                    if not cell_value or str(cell_value).strip() == "":
                        break
                    # Also check if we hit another group (shouldn't happen but safety check)
            ...
            print(f"INFO: {group_name}: rows {start_row}-{end_row}")
    ...
    def update_position_data(self, ws, account_group, account_info, positions_data, ticker_yields):
        """Update position data with proper row insertion and formatting"""
        account_key = self.account_groups.get(account_group)
        if not account_key or account_group not in account_info:
            print(f"ERROR: Unknown account group: {account_group}")
            return
        
        print(f"INFO: Updating position data for {account_group}...")
        
        # Get current positions from cache
        account_positions = self.get_positions_for_account(positions_data, account_key)
        
        if not account_positions:
            print(f"WARNING: No positions found for {account_group}")
            return
        
        print(f"INFO: Found {len(account_positions)} positions")
        ...
        print(f"INFO: Found {len(existing_tickers)} existing tickers")
        ...
        for row in range(start_row + 2, end_row + 1):
            ...
            if cell_value:
                ...
                if ticker in account_positions:
                    ...
                    print(f"UPDATE: Row {row}: {ticker} | Qty: {position_data['quantity']} | Price: ${position_data['current_price']:.2f}")
                    updated_count += 1
        ...
        if new_positions:
            print(f"INFO: Adding {len(new_positions)} new positions by inserting rows...")
            
            # Insert rows at the end of the group
            ...
            for ticker, position_data in new_positions.items():
                ...
                print(f"INSERT: Row {insert_at_row}: NEW {ticker} | Qty: {position_data['quantity']} | Price: ${position_data['current_price']:.2f}")
        ...
        print(f"SUCCESS: Updated {updated_count} position entries with proper formatting")
    
    def insert_new_yield_column(self, ws):
        """Insert new column after column O (15) for today's yields"""
        print("INFO: Inserting new yield column after column O...")
        ...
        print(f"SUCCESS: Inserted yield column P with date {self.today_str}")
        return new_col
    
    def update_yield_data(self, ws, account_group, account_info, new_col, ticker_yields):
        """Update yield data with actual percentages"""
        if account_group not in account_info:
            return
            
        print(f"INFO: Updating yields for {account_group}...")
                ...
                if ticker in ticker_yields:
                    ...
                    if prev_yield is not None:
                        ...
                        if dividend_yield > prev_yield_pct:
                            ...
                            color_desc = "YELLOW_SAME"
                    else:
                        yield_cell.fill = PatternFill(fill_type=None)
                        color_desc = "NO_COLOR"
                    
                    print(f"YIELD: Row {row}: {ticker} = {dividend_yield:.2f}% ({color_desc})")
                    updated_count += 1
                else:
                    print(f"WARNING: Row {row}: {ticker} - No yield data found")
        
        print(f"SUCCESS: Updated {updated_count} yields for {account_group}")
    ...
    def apply_group_divider_color(self, ws, account_info, new_col):
        ...
        for group_name, info in account_info.items():
            ...
            print(f"DIVIDER: {group_name} divider at row {row_num}")
    
    def run_update(self):
        """Run the Windows-compatible enhanced update"""
        print("WINDOWS-COMPATIBLE ENHANCED HISTORICAL YIELD UPDATER")
        print("=" * 55)
        ...
        # Step 5: Insert new yield column first
        print("\nSTEP 4: Inserting new yield column...")
        new_col = self.insert_new_yield_column(historical_sheet)
        
        # Step 6: Update position data for each group
        print("\nSTEP 5: Updating position data with proper formatting...")
        for account_group in account_info.keys():
            self.update_position_data(historical_sheet, account_group, account_info, positions_data, ticker_yields)
        
        # Step 7: Update yields
        print("\nSTEP 6: Updating yield data with actual percentages...")
        for account_group in account_info.keys():
            self.update_yield_data(historical_sheet, account_group, account_info, new_col, ticker_yields)
        ...
        try:
            ...
            print(f"SUCCESS: Successfully saved: {self.excel_file}")
        except Exception as e:
        ...
        print("\nSUCCESS: WINDOWS-COMPATIBLE UPDATE COMPLETED!")
        print("Updated: Tickers, Quantities, Prices, and Yields")
        print("Applied: Light blue formatting and proper row insertion")
        return True

def main():
    """Main execution function"""
    try:
        updater = WindowsCompatibleHistoricalYieldUpdater()
        success = updater.run_update()
        
        if success:
            print("\nSUCCESS: Windows-compatible historical yield update completed!")
        else:
            print("\nERROR: Windows-compatible historical yield update failed")
