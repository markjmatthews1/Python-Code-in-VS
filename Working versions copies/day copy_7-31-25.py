...
# --- Splash Popup (Vibrant, Always on Top) ---
_day_splash = tk.Tk()
...
def append_missing_schwab_data(historical_file, tickers, max_ticks=500):
    ...
    for ticker in tickers:
        ...
        if not hist_df.empty and ticker in hist_df["Ticker"].unique():
            last_dt = hist_df[hist_df["Ticker"] == ticker]["Datetime"].max()
...
historical_data = append_missing_schwab_data(HISTORICAL_DATA_FILE, tickers, max_ticks=500)  # Increased for better AI analysis
# diagnostic prints
...
def calculate_adx(df, period=14):
    """
    Calculate ADX, +DI, and -DI with proper initialization to avoid extreme starting values.
    """
    df = df.copy()
    ...
    # **FIXED: Use Wilder's smoothing (proper ADX calculation)**
    # Initialize smoothed values with first valid rolling average
    df['TR_smooth'] = np.nan
    df['+DM_smooth'] = np.nan
    df['-DM_smooth'] = np.nan
    
    # Calculate initial values using rolling mean for the first period
    initial_tr = df['TR'].iloc[1:period+1].mean()
    initial_plus_dm = df['+DM'].iloc[1:period+1].mean()
    initial_minus_dm = df['-DM'].iloc[1:period+1].mean()
    
    # Set initial smoothed values
    df.iloc[period, df.columns.get_loc('TR_smooth')] = initial_tr
    df.iloc[period, df.columns.get_loc('+DM_smooth')] = initial_plus_dm
    df.iloc[period, df.columns.get_loc('-DM_smooth')] = initial_minus_dm
    ...
    df['+DI'] = np.where(df['TR_smooth'] > 0, 100 * (df['+DM_smooth'] / df['TR_smooth']), 0)
    df['-DI'] = np.where(df['TR_smooth'] > 0, 100 * (df['-DM_smooth'] / df['TR_smooth']), 0)
    ...
    # Initialize ADX with first DX value after the smoothing period
    first_adx_idx = period * 2  # Need double the period for proper ADX initialization
    if first_adx_idx < len(df):
        df.iloc[first_adx_idx, df.columns.get_loc('ADX')] = df['DX'].iloc[period:first_adx_idx+1].mean()
...
def calculate_adx_multi(df, tickers, period=21):  # Set default to 21, ignore settings
    """
...
def start_dashboard(historical_data, filtered_df, tickers, dashboard_ranks):
    ...
    app.layout = html.Div([
        ...
        html.Div(id='trade-log-summary', style={'fontWeight': 'bold', 'fontSize': '16px', 'marginBottom': '10px'}),
    ...
    def update_dash(n, selected_tickers, n_clicks,
        ...
        # --- For display, slice last N rows and add Tick column ---
        pmo_plot_df = pd.concat([
            (
                pmo_full_df[pmo_full_df["Ticker"] == t].sort_values("Datetime").tail(pmo_tick_count)
                .assign(Tick=lambda x: range(1, len(x) + 1))
            )
            for t in valid_ticker_rows
        ], ignore_index=True)
        ...
        adx_df = calculate_adx_multi(adx_plot_df, valid_ticker_rows, period=21)
        ...
        def detect_candle_signals(tdf):
            ...
            for i in range(1, len(tdf)):
                ...
                if (c2 > o2 and c1 < o1 and o2 < c1 and c2 > o1):
                    signals.append({"Tick": tick, "Price": l2 * 0.995, "Signal": "Buy"})
                    
                # Bearish Engulfing: current red candle engulfs previous green candle
                elif (c2 < o2 and c1 > o1 and o2 > c1 and c2 < o1):
                    signals.append({"Tick": tick, "Price": h2 * 1.005, "Signal": "Sell"})
                ...
                if (total_range2 > 0 and body2 > 0 and 
                    ...
                    signals.append({"Tick": tick, "Price": l2 * 0.998, "Signal": "Buy"})
                ...
                # Shooting Star (bearish reversal): small body, long upper shadow
                    ...
                    signals.append({"Tick": tick, "Price": h2 * 1.002, "Signal": "Sell"})
                ...
                # Doji (indecision): very small body relative to shadows
                    ...
                    if c1 > o1:  # Previous was green (uptrend)
                        signals.append({"Tick": tick, "Price": h2 * 1.001, "Signal": "Sell"})
                    elif c1 < o1:  # Previous was red (downtrend)
                        signals.append({"Tick": tick, "Price": l2 * 0.999, "Signal": "Buy"})
        ...
        try:
            ...
            for i, ticker in enumerate(valid_ticker_rows, start=1):
                ...
                # Validate OHLC data
                required_chart_cols = ["Open", "High", "Low", "Close", "Tick"]
                ...
                if signals:
                    ...
                    if sell_signals:
                        price_fig.add_trace(
                            go.Scatter(
                                x=[s["Tick"] for s in sell_signals],
        ...
        for i, ticker in enumerate(valid_ticker_rows, start=1):
            ...
            if not adx_sub.empty and "ADX" in adx_sub.columns and "+DI" in adx_sub.columns and "-DI" in adx_sub.columns:
                ...
                # Remove NaN values for smoother lines
                adx_clean = adx_sub.dropna(subset=["ADX", "+DI", "-DI"])
                ...
                if not adx_clean.empty:
                    ...
                    # Add reference lines
                    max_val = max(adx_clean["ADX"].max(), adx_clean["+DI"].max(), adx_clean["-DI"].max())
                    min_val = min(adx_clean["ADX"].min(), adx_clean["+DI"].min(), adx_clean["-DI"].min())
