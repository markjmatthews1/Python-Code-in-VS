...
def calculate_adx_multi(df, tickers, period=14):  # Reduce default period for intraday
    """
    ...
    print(f"🔍 ADX_MULTI DEBUG: Period: {period}")
    ...
    for ticker in tickers:
        ...
        if len(tdf) < period * 2:
            print(f"⚠️ ADX_MULTI WARNING: {ticker} has insufficient data ({len(tdf)} < {period * 2})")
            ...
            if adx_df is not None and not adx_df.empty:
                print(f"✅ ADX_MULTI DEBUG: {ticker} ADX calculation successful")
    ...
    if results:
        ...
        print(f"🔍 ADX_MULTI DEBUG: Final result shape: {final_result.shape}")
    ...
    def update_dash(n, selected_tickers, n_clicks,
        ...
        def get_last_n_with_datetime(df, ticker, n):
            ...
            if 'Datetime' in tdf.columns and not tdf.empty:
                tdf['Date'] = pd.to_datetime(tdf['Datetime'], format='mixed').dt.date
                # Try to get today's data first
                today_data = tdf[tdf['Date'] == today].copy()
        ...
        # **FIX ADX**: Use shorter period for intraday data to ensure we have enough data points
        intraday_adx_period = min(14, max(7, len(adx_plot_df) // 4))  # Dynamic period based on available data
        print(f"📊 ADX DEBUG: Using period {intraday_adx_period} for intraday ADX calculation")
        ...
        for ticker in valid_ticker_rows:
            ...
            print(f"📊 ADX DEBUG: {ticker} has {len(ticker_adx_data)} data points for ADX calculation")
        
        adx_df = calculate_adx_multi(adx_plot_df, valid_ticker_rows, period=intraday_adx_period)
        print(f"📊 ADX DEBUG: ADX calculation result shape: {adx_df.shape}")
