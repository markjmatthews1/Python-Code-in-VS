...
def get_schwab_individual_positions():
    """Get current positions from Schwab Individual account using existing auth system."""
    try:
        print("🔄 Getting Schwab Individual account positions...")
        ...
        # Find the Individual account by account number (7450-1314)
        individual_account_hash = None
        target_account_number = "7450-1314"
        
        for account in accounts:
            # Check multiple possible fields for account number
            account_number = account.get('accountNumber', '') or account.get('accountId', '')
            account_hash = account.get('hashValue', '')
            
            if target_account_number in str(account_number):
                individual_account_hash = account_hash
                print(f"✅ Found Schwab Individual account: {account_number} (hash: {account_hash})")
                break
        
        if not individual_account_hash:
            print(f"❌ Could not find Schwab Individual account {target_account_number}")
            # Show all accounts for debugging
            for account in accounts:
                account_number = account.get('accountNumber', '') or account.get('accountId', '') or 'Unknown'
                account_hash = account.get('hashValue', '')
                print(f"   Available: {account_number} (hash: {account_hash})")
            return []
        
        # Get positions for the Individual account
        positions_url = f"{base_url}/trader/v1/accounts/{individual_account_hash}/positions"
        ...
        print(f"📈 Found {len(positions)} positions in Schwab Individual account")
...
def update_excel_sheet(annual_income):
    ...
    try:
        ...
        # Add the annual income to row 7 (Schwab Individual) with proper formatting
        format_dividend_cell(ws, 7, target_col, annual_income)
